...
?>
    ...
    <head>
        ...
        <title>TSRadioBot &middot Bot Controller</title>
    </head>
    ...
    <body>
      ...
      <i class="material-icons">dashboard</i> Dashboard
        ...
        <i class="material-icons">format_list_bulleted</i> Musikbots</a>
                        </div>
                        <?php
                        if($groupid == "#1337"){
                            echo '<div class="sidebar-nav">';
                            echo '<a href="#" class="sidebar-nav-link">';
      ...
      <i class="material-icons">power_settings_new</i>
                        ...
                        <a href="botcreator.php" align="right" data-toggle="tooltip" title="Bot erstellen"><i align="right" class="material-icons">build</i></a>
                    </div>
                    <hr />
                    <?php
                    if($groupid == '#1337'){
    ...
    <i class="fa fa-bars"></i>
                        ...
                        <div class="navbar-brand">
                            TSRadioBot &middot Bot Controller
                        </div>
                    </nav>
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Bot Controller</li>
                        </ol>
                    </nav>
                    <div class="container-fluid">
                        <div class="row">
                            <div class="tablebox">
                                <h2 class="box-title" align="center"><i class="material-icons">video_library</i> Verfügbare Bots</h2>
                                <hr />
                                <input type="text" id="inputsearch" onkeyup="search()" placeholder="Kunden ID suchen..." align="left">
                                <input type="text" id="inputsearch2" onkeyup="search2()" placeholder="BotID suchen..." align="left">
                                <input type="text" id="inputsearch3" onkeyup="search3()" placeholder="IP suchen..." align="left">
                                <table class="tables" id="tables">
                                    ...
                                    while($row = mysqli_fetch_array($result)){
                                        ...
                                        $outer = 'Status: <span class="badge badge-warning">Unbekannt</span>'; $outer = 'Status: <span class="badge badge-warning">Unbekannt</span>'; $ipadress = strtolower($system).".tsradiobot.de"; $fp = fsockopen($ipadress, $row['Port'], $errno, $errstr, 30); if (!$fp) { $outer = 'Status: <span class="badge badge-danger">Offline</span>'; } else { $outer = 'Status: <span class="badge badge-success">Online</span>'; }
                                        echo "<td>" .$status."</td>"; 
                                        echo "<td>" . urldecode($row['IP']) . "</td>";
                                        $botid = $row['BotID'];
                                        echo "<td>".$outer."</td>";
                                        echo '<td><form action="botadmin.php" method="post"><input type="hidden" name="port" value='.$row['Port'].'><input type="hidden" name="bot" value='.$row['BotID'].'> <button type="submit" class="btn"> Optionen</button> </form></td>';
            ...
            <?php 
            if($_GET['answer'] == "deleted"){
               echo('<script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script><script>swal("Erfolgreich", "Der Bot wurde gelöscht.", "success");</script>');
                }
            ?>
    </body>
